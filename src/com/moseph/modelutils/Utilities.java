...
{
	/*
	 * Slightly ugly chunk of code, to make it easy to use Repast's repeatable RNGs if they are present
	 * and the system RNG otherwise
	 */
	static RandHelp randHelp = new SystemRandomHelper();
	public static double nextDoubleFromTo( double start, double end ) { return randHelp.nextDoubleFromTo( start, end ); }
	public static double nextDouble() { return randHelp.nextDouble(); }
	public static int nextIntFromTo( int start, int end ) { return randHelp.nextIntFromTo( start, end ); }
	{
		try
		{
			randHelp = new RepastRandomHelper();
			System.out.println("Using Repast RNG");
		} catch( Exception e ) { System.out.println("No Repast RNG found");}
	}
	public static void setRandomHelper( RandHelp r ) { randHelp = r; }
	public static void setSeed( long seed ) { randHelp.setSeed( seed ); }
	
	
	public static <T> T sample( Map<T, Double> probabilities )
	{
		return sample( probabilities, false );
	}

	/**
	 * Samples from the map of probabilities (i.e. T -> prob of T)
	 * 
	 ...
	 * @return
	 */
	public static <T> T sample( Map<T, Double> probabilities, boolean forceNormalised )
	{
		double current = 0;
		double maxProb = 1;
		if( forceNormalised )
		{
			...
			for ( double d : probabilities.values() )
				maxProb += d;
			if( maxProb == 0 )
				return sample(probabilities.keySet());
		}
		double random = nextDoubleFromTo(0,maxProb);
	...
	/**
	 * Gives the number of events from a given population and probability. The idea is
	 * that sometimes we'd like to do this non-stochastically, to make testing
	 * easier. In that case, it's (int)(population*prob). Otherwise, it samples
	 * the population at "prob".
	 * 
	 ...
	 * @return
	 */
	public static int applyProbability( int population, double rate, boolean stochastic )
	{
		if( stochastic )
		{
			//TODO: appalling lazy, Bernoulli would kill me but it's Saturday morning and
			...
			return (int) ( rate * (double) population );
	}
	
	/**
	 * Used when we need to have discrete events happen at an average rate
	 * 
	 * Not very sophisticated - ought to include variance, but this will at least
	 * give approximate rates
	 * @param rate
	 * @return
	 */
	public static int sampleRate( double rate )
	{
		int value = (int)floor( rate );
		if( nextDouble() < rate - value ) value++;
		return value;
	}

	/**
	 * Given a floating point rate at which events should occur and a population
	 * returns a number of events. By converting the target rate into a probability
	 * of occurrence based on the population size.
	 * 
	 * @param population
	 * @param targetRate
	 * @return
	...
	{
		double rate = (double)targetRate/population;
		return applyProbability( population, rate, true );
	}
	...
	public static <T> T consume( Map<T, Integer> population )
	{
		...
		while ( it.hasNext() )
			if( it.next().getValue() <= 0 )
				it.remove();
		T value = sample( population.keySet() );
		if( value == null )
			return null;
	...
	{
		int index = nextIntFromTo( 0, population.size() -1 );
	...
	{
		int num = nextIntFromTo( 0, objects.size() - 1 );
	...
	/**
	 * Returns a list of the keys of the map, sorted by their values, with the highest value first
	 * 
	 * @param <T>
	 * @param input
	 * @return
	...
	public static <T> List<T> sortMap( final Map<T, ? extends Number> input )
	{
		Comparator<T> c = new Comparator<T>()
		{
			...
			{
				return -Double.compare( input.get( o1 ).doubleValue(), input.get( o2 ).doubleValue() );
			}
		};
	...
	/**
	 * Returns up to the specified number of samples from the input.
	 * Get N items from a list. Alternate formulation to sampleN( Collection, number )
	 * as it avoids copying the input
	 * @param <T>
	 * @param input
	 * @param toSample
	 * @return
	...
	public static <T> Set<T> sampleN( Collection<T> input, int toSample )
	{
		...
		while ( toSample > 0 && it.hasNext() )
		{
			T cur = it.next();
			//-1 as it's inclusive
			int rand = nextIntFromTo( 0, nLeft-1 );
			if( rand < toSample )
			{
				ret.add( cur );
				toSample--;
	...
	/**
	 * A simple interface for something which gets a numeric value from a certain type of object
	 * @author dmrust
	 *
	 * @param <T>
	 */
	...
	public static <T> Map<T, Double> scoreMap( Collection<T> items, Score<T> score )
	{
		Map<T, Double> map = new HashMap<T, Double>();
		for( T t : items ) map.put( t, score.getScore( t ) );
		return map;
	}
	
	/**
	 * Creates a comparator based on calculating a score for each object and storing the resutls
	...
	public static class LazyScoreComparator<T> implements Comparator<T>
	{
		...
		{ 
			if( ! scores.containsKey( arg0 ) ) scores.put(arg0,score.getScore( arg0 ));
			if( ! scores.containsKey( arg1 ) ) scores.put(arg1,score.getScore( arg1 ));
	...
	{
		Map<T, Double> scores;
		public ScoreComparator( Collection<T> items, Score<T> score ) { scores = scoreMap( items, score ); }
		public ScoreComparator( Map<T,Double> scores ) { this.scores = scores; }
		public int compare( T arg0, T arg1 )
		{ 
			return scores.get(arg0).compareTo( scores.get(arg1) ); 
		}
	...
	{
		int i = nextIntFromTo( 0, collection.size() - 1 );
		int index = 0;
		for( T t : collection )
		{
			index++;
	...
	public static <P,T extends P> void incrementHash( Map<P, Double> map, T key, double amount )
	{
	...
	public static <P,T extends P> void incrementHash( Map<P, Integer> map, T key, int amount )
	{
		if( map.containsKey(key)) map.put( key, map.get(key) + amount );
		else map.put(key, amount);
	}
	...
	public static <P> P argmax( Map<P, ? extends Number> map )
	{
		...
		for( P p : map.keySet() )
			if( map.get( p ).doubleValue() > max )
				{ val = p; max = map.get( p ).doubleValue(); }
		return val;
	}
	 ...
	 */
	public static <P, T extends P> void rateUpdate( Map<P,Double> map, T key, double rate, double update, boolean ignoreZero, boolean ignoreNegative )
	{
		double current = hashDouble( map, key );
		map.put( key, rateUpdate( current, update, rate, ignoreZero, ignoreNegative ) );
	}
	
	/**
	 * Updates the given number with the update provided, and the rate specified. Rate of 0 = use existing value, rate of 1 = take the new number
	 * rate 0.5 is halfway between
	 * @param current
	 * @param update
	 * @param rate
	 * @param ignoreZero
	 ...
	 * @return
	 */
	public static double rateUpdate( double current, double update, double rate, boolean ignoreZero, boolean ignoreNegative )
	{
		if( ignoreZero && current == 0 ) return update;
		if( ignoreNegative && current < 0 ) return update;
	...
	class RepastRandomHelper implements RandHelp
	{
		...
		Method nextDbl = null;
		Method setSeed = null;
		public RepastRandomHelper() throws ClassNotFoundException, SecurityException, NoSuchMethodException
		{
			Class<?> clazz = Class.forName( "repast.simphony.random.RandomHelper" );
			dblFrom = clazz.getDeclaredMethod( "nextDoubleFromTo", Double.TYPE, Double.TYPE );
			intFrom = clazz.getDeclaredMethod( "nextIntFromTo", Double.TYPE, Double.TYPE );
			nextDbl = clazz.getDeclaredMethod( "nextDouble");
			setSeed = clazz.getDeclaredMethod( "setSeed");
		}
		public double nextDoubleFromTo( double start, double end )
		{ 
			try { return ((Double)dblFrom.invoke(null, start, end)).doubleValue(); } 
			catch (Exception e) { e.printStackTrace(); return 0;} 
		}
		public int nextIntFromTo( int start, int end )
		{ 
			try { return ((Integer)intFrom.invoke(null, start, end)).intValue(); } 
			catch (Exception e) { e.printStackTrace(); return 0;} 
		}
		public double nextDouble()
		{ 
			try { return ((Double)nextDbl.invoke(null)).doubleValue(); } 
			catch (Exception e) { e.printStackTrace(); return 0;} 
		}
		/* Untested */
		public void setSeed(long seed )
		{
			try { setSeed.invoke(seed); } 
